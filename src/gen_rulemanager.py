...
Some conversion capabilities may not works. see http://github.com/ltn22/pyang""".format(name)) 
    ...
    def sid_search_sid(self, value, short=False):
        ...
        for s in self._sid_info:
            name = s["ietf-sid-file:sid-file"]["module-name"]
    ...
    def get_yang_type (self, yangid):
        for s in self._sid_info:
            module_name = s["ietf-sid-file:sid-file"]['module-name']
            for e in s["ietf-sid-file:sid-file"]['item']:
    ...
    def convert_to_json(self, jcc, delta=0, name_ref=""):
        if type(jcc) is dict:
            ...
            return json_list
        elif type(jcc) in [int, str]:
            if type(jcc) is str: # large number in JSON are strings
                jcc = int(jcc)
