...
class RuleManager:
    ...
    def _create_compression_rule (self, nrule, device_id = None):
        ...
        for r in nrule[T_COMP]:
            ...
            univ_opt = False
            if self.universal_option and r[T_FID].upper().find('COAP_OPTION')==0:
                univ_opt = True
                pass
            elif not r[T_FID].upper() in FIELD__DEFAULT_PROPERTY:
                raise ValueError( "Unkwown field id {} in rule {}/{}".format(
                    r[T_FID], arule[T_RULEID],  arule[T_RULEIDLENGTH]
                ))

            entry = {}
            if not univ_opt:
                from compr_parser import option_names, coap_options
                ...
                FID = r[T_FID].upper()
                entry[T_FID] = FID         
                entry[T_FL] = self._return_default(r, T_FL, T_FUNCTION_VAR)
    ...
    def __init__(self, file=None, log=None, universal_option=False):
        #RM database
        self._ctxt = []
        self._log = log
        self._db = []
        ...
        self.universal_option=universal_option
            ...
            for rule in dev["SoR"]:
                ...
                if T_COMP in rule:
                    ...
                    for e in rule[T_COMP]:
                        ...
                        if self.universal_option and e[T_FID].find("COAP_OPTION:")==0:
                            ...
                            # "/ietf-schc:schc/rule/ietf-schc-opt:entry-option-space/field-length"
                            l=e[T_FL]
                            if type(l) == int:
                                entry_cbor += \
